...
{
    double effectiveDiffusion { biMolData.Dtot };
    double Dr1 {};
    if (std::abs(complexList[biMolData.com1Index].D.z - 0) < 1E-10) {
        ...
        effectiveDiffusion += Dr1 / (4.0 * params.timeStep);
    } else {
        double cf = cos(sqrt(4.0 * complexList[biMolData.com1Index].Dr.z * params.timeStep));
        Dr1 = 2.0 * biMolData.magMol1 * (1.0 - cf);
        effectiveDiffusion += Dr1 / (6.0 * params.timeStep);
    }
    ...
    if (std::abs(complexList[biMolData.com2Index].D.z - 0) < 1E-10) {
        ...
        effectiveDiffusion += Dr2 / (4.0 * params.timeStep);
    } else {
        double cf = cos(sqrt(4.0 * complexList[biMolData.com2Index].Dr.z * params.timeStep));
        Dr2 = 2.0 * biMolData.magMol2 * (1.0 - cf);
        effectiveDiffusion += Dr2 / (6.0 * params.timeStep);
    }

    double rMax { 3.0 * sqrt(6.0 * effectiveDiffusion * params.timeStep)
        + forwardRxn.bindRadius };
    return { effectiveDiffusion, rMax };
}
    ...
    const std::vector<BackRxn>& backRxns)
{
    // TRACE();
    /*3D reaction*/
    const Bimolecular3DParameters parameters =
        calculate_3D_bimolecular_parameters(
            biMolData, params, complexList, forwardRxns[rxnIndex]);
    biMolData.Dtot = parameters.effectiveDiffusion;

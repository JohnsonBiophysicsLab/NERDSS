    ...
    for (auto& oneRxn : createDestructRxns) {
        ...
        if (oneRxn.rxnType == ReactionType::uniMolCreation) {
            ...
            for (auto& oneMol : moleculeList) {
                ...
                // out-of-bounds Complex, which is a vector header made of
                // whatever the neighbouring allocation held.  A destroyed
                // molecule is not a reactant in any case, so it is skipped before
                // the complex is looked up; && evaluates left to right, so the
                // argument is never formed.

    ...
    std::vector<Molecule>& moleculeList, bool isSphere)
{
    ...
    if (complexList[moleculeList[pro1].myComIndex].onFiber && complexList[moleculeList[pro2].myComIndex].onFiber) {
        // The 1D case is quite special, we do not use this routine for 1D reactions unless its within the same complex
        // Check out src/reactions/determine_1D_bimolecular_reaction_probability.cpp for implementation
        is1D = true;
